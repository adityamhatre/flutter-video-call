...
class ConnectycubeFlutterCallKitPlugin : FlutterPlugin, MethodCallHandler,
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        println("callidtoprocess, $callIdToProcess")
    ...
    private fun saveCallState(callId: String, callState: String) {
        println("appcontext $applicationContext")
        if (applicationContext == null) return
        if (callState == CALL_STATE_ACCEPTED) {
            putString(applicationContext!!, "flutter.callId", callId)
        }
        if (callState == ACTION_CALL_REJECT) {
            removeString(applicationContext!!, "flutter.callId")
        }
